...
public class MainActivity extends AppCompatActivity implements RealmChangeListener<RealmResults<UserContact>> {

    Realm realm;
    Adapter adapter;
    ListView listView;
    RealmResults<UserContact> results;
    boolean asc = true;
    ...
    void load(){
        results = realm.where(UserContact.class).findAll();
    ...
    private void createNewContact(String name,int edad, boolean genero){
        ...
        UserContact contact = new UserContact(1+System.currentTimeMillis(),name,edad,genero);
    ...
    private void dialogoAdd(){
        ...
        final EditText edad = viewInflate.findViewById(R.id.age);
        final ToggleButton sexo = viewInflate.findViewById(R.id.gender);
    ...
    boolean gen=true;
    void showMorF(){
        results = realm.where(UserContact.class)
                .equalTo("genero",gen)
                .findAll();
        gen=!gen;
        adapter = new Adapter(this,results,R.layout.item_contact);
    ...
    @Override
    public void onChange(RealmResults<UserContact> element) {
